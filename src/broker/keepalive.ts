...
/** 주기 sweep을 시작한다 — 타이머는 unref로 프로세스 종료를 막지 않는다. */
export function startKeepalive(registry: Registry, intervalMs: number): Timer {
  const timer = setInterval(() => sweepKeepalive(registry), intervalMs);
  timer.unref();
  return timer;
}
