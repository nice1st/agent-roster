...
test("오버라이드가 없으면 기본값 30초·1000연결로 해석된다", () => {
  expect(resolveHygiene()).toEqual({ heartbeatIntervalMs: 30_000, maxConnections: 1_000 });
});
...
test("BROKER_ env 2종을 숫자 오버라이드로 파싱한다", () => {
  ...
  expect(hygieneFromEnv(env)).toEqual({ heartbeatIntervalMs: 5000, maxConnections: 10 });
});
...
test("0 이하의 env 값은 거부한다", () => {
  expect(() => hygieneFromEnv({ BROKER_HEARTBEAT_INTERVAL_MS: "0" })).toThrow();
});
