...
```
사용자 ─소유─> 에이전트(CC·OpenCode) ─regi─> 브로커 ──발견(그룹)·전달(UUID)──> 다른 에이전트
...
| 엔티티 | 요지 |
|--------|------|
| user | 콘솔 사용자. 관리자가 email로 생성 → 초대 → 당사자가 Google 로그인으로 수락해 활성화(초대 안 된 계정은 로그인 불가). 삭제도 관리자 몫이며 자기 자신은 삭제할 수 없다. 관리자도 user이며 사용자·그룹 관리 권한만 추가로 가진다 |
| agent | 사용자가 로컬에서 띄운 대화형 코딩 에이전트 세션(Claude Code·OpenCode — 클라이언트별 플러그인은 [architecture §4](architecture.md)). **영속하지 않는다** — 정체성은 regi 시 브로커가 발급하는 UUID이며 레지스트리(메모리)에만 존재한다(§3.1). user가 소유. regi 시 표시용 메타(머신·작업 디렉토리·별칭·상태)를 함께 싣는다 |
...
- **토큰 = user 정체성만(JWT)** — 토큰은 "이 요청이 어느 user냐"만 담아 서명된 JWT다. 검증만으로 user를 알며 DB 조회가 필요 없다. 발급 토큰을 저장하지 않으므로 폐기(revoke)는 1차에 없고, 만료도 사실상 두지 않는다(초장기). 유출 시 대응은 **서명키 회전** — 전체 토큰이 한꺼번에 무효화되고 모두가 재발급받는다. 그룹은 DB의 user_group에서 파생하므로, user가 그룹에 들고나도 토큰 재발급이 필요 없다. 토큰의 유일한 사용처는 regi다.
...
- **웹 세션은 노출 없는 엔트리** — 웹 앱이 열려 있는 동안 수신을 위해 레지스트리에 등재되지만(연결 1개, [architecture §5](architecture.md)), 노출 그룹이 없어 어떤 peers에도 보이지 않는다. 송신은 격리하지 않으므로 에이전트의 답장은 정상 도착한다. 에이전트가 사용자에게 먼저 말을 거는 역방향은 1차에 없다 — 웹은 사용자가 연 대화의 상대에게서 온 수신만 표시하고 그 외 from은 버린다.
...
- **연결 위생** — Map이 무한히 커지거나 죽은 엔트리가 남지 않도록: ① 연결 종료(`close`/`cancel`) 시 즉시 제거 + keepalive push 실패 감지, ② 프로세스당 연결 수 상한. 수치와 두지 않기로 한 장치(유휴 타임아웃·미전송 큐 상한)의 근거는 [architecture §4](architecture.md).
...
- **발신자 제외 팬아웃** — 발언은 room UUID를 대상으로 보내고, 발신자를 제외한 참여자 전원에게 전달된다(발신 확인은 도구 응답이 담당). 특정 상대 지정 발언은 없다. 1:1 send와는 별도 인터페이스다 — "기록되는 발언"임이 호출자에게 명시되어야 한다([plugin §2](plugin.md)).
...
**브로커는 지시가 오가는 통로까지다.** 메시지를 받은 에이전트가 무엇을 하는지는 사용자 ↔ 에이전트의 영역이며 브로커가 제어·관여하지 않는다.

- 에이전트는 사용자가 정한 로컬에서 구동 중이다. 역할·권한·읽기쓰기 범위는 전부 에이전트 클라이언트 쪽 설정 — 브로커는 알지 못한다.
- room · 1:1 · 로컬터미널은 **같은 에이전트에게 지시가 도착하는 세 경로**일 뿐. 컨텍스트 유지는 에이전트 클라이언트가 담당한다.
