...
- **전체 팬아웃** — 발언은 room UUID를 대상으로 보내고, 발신자를 제외한 참여자 전원에게 전달된다(발신 확인은 도구 응답이 담당). 특정 상대 지정 발언은 없다. 1:1 send와는 별도 인터페이스다 — "기록되는 발언"임이 호출자에게 명시되어야 한다([plugin §2](plugin.md)).
