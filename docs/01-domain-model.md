...
- **연결 위생** — Map이 무한히 커지거나 죽은 엔트리가 남지 않도록: ① 연결 종료(`close`/`cancel`) 시 즉시 제거 + keepalive push 실패 감지, ② 프로세스당 연결 수 상한. 수치와 두지 않기로 한 장치(유휴 타임아웃·미전송 큐 상한)의 근거는 [05 §4](05-plan.md).
