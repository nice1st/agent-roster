...
- **웹 UI = React + TypeScript, Bun 내장 번들링(HTML import)** — 실시간(SSE 반영) 화면이라 SPA, Better Auth react 클라이언트 사용. 분리 대비 습관: `web/`은 서버 코드를 import하지 않고(타입도 `web/types.ts`에 자체 정의) API 호출은 상대경로 fetch만 쓴다. 폴백은 `bun build` 정적 산출물.
